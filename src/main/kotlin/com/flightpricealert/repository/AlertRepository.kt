    ...
    fun create(origin: String, destination: String, targetPrice: Double, airlines: List<String>?): FlightAlert {
        ...
        return DatabaseFactory.useConnection { connection ->
            connection.prepareStatement(
                """
                INSERT INTO flight_alerts (origin, destination, target_price, airlines, active, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
                """.trimIndent(),
                ...
                statement.setString(4, airlinesValue)
    ...
    fun update(id: Int, origin: String, destination: String, targetPrice: Double, airlines: List<String>?, active: Boolean): FlightAlert? {
        ...
        return DatabaseFactory.useConnection { connection ->
            connection.prepareStatement(
                ...
                SET origin = ?, destination = ?, target_price = ?, airlines = ?, active = ?
                WHERE id = ?
                """.trimIndent()
            ).use { statement ->
                statement.setString(1, origin)
